...
class CoinfloorWSS(websocket.WebSocketApp):
...
def test_drive():
    if len(sys.argv) < 2:
        print("Usage: coinfloorClient.py ws[s]://<host>[:<port>]")
        return -1

    def print_out(ws, msg):
        print("%s: %s"%(time.time(), msg))

    coinfloor = CoinfloorWSS(
            sys.argv[1],
            #self_signed   = True,
    ...
    #coinfloor.set_auth_data(<core_id>, <cookie>, <passphrase>)
